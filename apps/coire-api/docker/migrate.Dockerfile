...
#
# coire-migrate — one-shot Alembic runner.
#
# Migrations never run inside a long-lived service (FR-010). This container runs to completion
# and exits 0; coire-api depends on service_completed_successfully.
#
# No HEALTHCHECK: a one-shot container has nothing to keep healthy.
...
CMD ["-m", "alembic", "-c", "/app/alembic.ini", "upgrade", "head"]
