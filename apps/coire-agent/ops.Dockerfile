...
RUN uv venv --python 3.13 --relocatable /app/.venv && uv sync --frozen --no-dev --no-editable --no-install-workspace --package coire-agent
COPY packages/coire-core packages/coire-core
COPY apps/coire-agent apps/coire-agent
RUN uv sync --frozen --no-dev --no-editable --package coire-agent && cp -R apps/coire-agent/ops/coire_ops /app/.venv/lib/python3.13/site-packages/ && rm -rf /usr/local/bin/pip* /usr/local/lib/python3.13/site-packages/pip* /app/.venv/bin/pip*
