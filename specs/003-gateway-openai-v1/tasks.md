...
- [ ] T003 [P] Add gateway test factories and fake streaming-engine fixtures in `apps/coire-api/tests/conftest.py`
...
- [ ] T012 [P] [US1] Add contract tests for `/v1/models` and OpenAI chat response/SSE shapes in `apps/coire-api/tests/contract/test_gateway_v1.py`
...
- [ ] T016 [US1] Register the `/v1` router and RFC 9457 error mapping in `apps/coire-api/src/coire_api/app.py`
- [ ] T017 [US1] Add official OpenAI SDK compatibility coverage in `tests/integration/test_gateway.py`
...
- [ ] T023 [P] [US3] Add load coordination, timeout, failure, and Retry-After tests in `apps/coire-api/tests/unit/test_gateway_loading.py`
- [ ] T024 [US3] Implement per-model single-flight load coordination and wait ceilings in `apps/coire-api/src/coire_api/gateway/loading.py`
- [ ] T025 [US3] Add stream keep-alives and wait/opt-out handling to `apps/coire-api/src/coire_api/routes/v1.py`
- [ ] T026 [US3] Add cold-load integration coverage to `tests/integration/test_gateway.py`
...
- [ ] T031 [P] [US5] Add usage finalization and exactly-once tests in `apps/coire-api/tests/unit/test_gateway_usage.py`
- [ ] T032 [P] [US5] Add disconnect and engine-failure streaming contract tests in `apps/coire-api/tests/contract/test_gateway_v1.py`
- [ ] T033 [US5] Integrate usage finalization and prompt/completion counters through all route outcomes in `apps/coire-api/src/coire_api/routes/v1.py`
...
- [ ] T035 [P] Add gateway dashboard panels in `deploy/compose/grafana/provisioning/dashboards/coire-gateway.json`
- [ ] T036 [P] Add overload and engine-failure alert rules in `deploy/compose/prometheus/rules/coire-gateway.yml`
- [ ] T037 [P] Document configuration, diagnosis, cancellation, kill, and rollback in `docs/runbooks/gateway.md` and `deploy/compose/README.md`
- [ ] T038 Regenerate OpenAPI and TypeScript API types in `apps/coire-api/openapi.json` and `apps/coire-web/src/api/schema.d.ts`
- [ ] T039 Run format, Ruff, mypy, unit/contract, OpenAPI freshness, web, and tiny-model integration gates from `specs/003-gateway-openai-v1/quickstart.md`
