...
- [ ] T005 [P] Update `apps/coire-node/install.sh` (AGENT_VERSION 0.2.0) to create `$PREFIX/models`, `$PREFIX/state/jobs`, `$PREFIX/hf-cache`; read the HF token from the System keychain at agent start is the agent's job, but `install.sh` prints the `coire-hf-token` command alongside the node-token one; update `apps/coire-node/uninstall.sh --dry-run` to enumerate the three new directories and the second Keychain item; update the `--dry-run` expectations in `docs/runbooks/bootstrap.md` §Node agent
...
- [ ] T008 [P] Create `packages/coire-core/src/coire_core/models/registry.py`: `ModelState`, `Visibility`, `Tag`, `PlacementPolicy` (validated `str` subtype with the regex from registry-api.yaml), `CapabilityProfile`, `Model`, `ModelCopy`, `ModelDetail`, `ModelListing`, `ModelAddRequest`, `ModelUpdateRequest` (rejects `capability_profile.verified`), `ModelRejected`, `LoadRefused`, `slug_for(repo_id)`; unit tests in `packages/coire-core/tests/test_registry_models.py` covering placement regex, tag enum, slug derivation, and the `verified` rejection
...
- [ ] T040 [US1] Wire `HF_TOKEN` for the Linux integration job: `.github/workflows/ci.yml` `integration` step passes `HF_TOKEN: ${{ secrets.HF_TOKEN }}` **only** into the compose override's `node-a` environment (T023) and asserts via `docker compose config` that no other service carries it; document in the workflow that the ungated test model works without it and the secret is optional
...
- [ ] T059 [US4] Extend the reconciler (T038/T051) in `apps/coire-api/src/coire_api/registry/reconciler.py` with `reconcile_node(node)`: build `expected` from non-terminal `EngineProcessRow`s for that node, call `reconcile`, then adopted → mirror `pid`/`create_time`/`state`; dead → `failed` with `state_reason="process gone during agent restart"` + audit `engine.reconcile`; orphans → insert `EngineProcessRow(state=orphan, model_id=None or matched by slug, node, port, pid)` if none exists for that `(node, pid)`; trigger it from `register_node` in `routes/nodes.py` (schedule via `app.state.reconciler.request_reconcile(name)`) and on every prober transition `unreachable → healthy`; make `model_id` nullable on `engine_processes` in `0002_registry.py` for orphans
...
- [ ] T061 [P] [US5] Contract tests in `apps/coire-api/tests/contract/test_registry_api.py`: `PATCH /api/v1/admin/models/{id}` (200 `ModelDetail`; 409 publishing a non-`ready` model; 422 on `capability_profile.verified`, a bad tag, a bad placement), `POST …/{id}/retire` (202; 409 already retired), `GET /api/v1/models` returns `ModelListing` only (assert no `copies`, `store_path`, `state_reason` keys) — anonymous sees published+ready with empty entitlement; admin sees everything
...
- Every `POST /node/jobs/*` and `POST /node/engines` is idempotent on the caller's id; the reconciler relies on that after a restart — do not "optimise" it away
