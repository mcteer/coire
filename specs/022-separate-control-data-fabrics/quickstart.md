...
- Remaining acceptance dependencies: representative tool-loop and image-result surfaces are not
  ...
  yet. The Prometheus/Alertmanager runtime is also not part of the deployed core compose project,
  so alert-rule syntax and unit fixtures pass but live pending/firing transitions cannot be
  recorded. These gates are not recorded as passed.
...
- Wi-Fi failure and recovery: disabling edge-b Wi-Fi made only edge-b unreachable and changed
  aggregate health to `degraded`; edge-a stayed healthy and served a first token in 756.9 ms. The
