...
- [X] T003 [P] Add run-orchestration settings with safe bounds and documented defaults in `packages/coire-core/src/coire_core/settings.py` and `deploy/compose/README.md`
...
- [X] T009 [P] Extend stable RFC 9457 run error codes in the shared run contract and their gateway mapping tests (adapted to this repository's contract-owned error-code layout)
...
- [X] T012 [P] [US1] Write control-plane run create/get/events contract tests and ownership checks in `apps/coire-api/tests/contract/test_runs.py`
...
- [X] T016 [US1] Extend the control-plane node client with typed run verbs and bounded streaming in `apps/coire-api/src/coire_api/nodes_client.py`
- [X] T017 [US1] Implement authenticated owner/admin run create/get/events routes and services in `apps/coire-api/src/coire_api/routes/runs.py`, `apps/coire-api/src/coire_api/runs.py`, and `apps/coire-api/src/coire_api/app.py`
- [X] T018 [US1] Implement deterministic Studio placement and a DBOS create/start/log/wait/collect/remove workflow in `apps/coire-api/src/coire_scheduler/runs.py` and dispatch it from `apps/coire-api/src/coire_scheduler/main.py`
- [X] T019 [US1] Record node, container, timings, exit status, resource usage, bounded logs, strict result, transitions, and terminal audit rows in `apps/coire-api/src/coire_scheduler/runs.py`
...
- [X] T020 [P] [US2] Write placement tests that reject core/non-Studio nodes and never fall back when Studios are unavailable in `apps/coire-api/tests/unit/test_run_placement.py`
- [X] T021 [P] [US2] Write Compose and image topology tests proving scheduler/API lack Docker sockets and core need not hold the user image in `tests/integration/test_run_core_isolation.py`
- [X] T022 [US2] Restrict placement candidates to healthy registered Studio nodes with sandbox capacity in `apps/coire-api/src/coire_scheduler/runs.py`
- [X] T023 [US2] Add explicit production-compose and image-policy guards against user-run services or Docker socket mounts on core in `deploy/compose/compose.yaml` and `scripts/image-policy.sh`
...
- [X] T025 [P] [US3] Write relay allowlist, header filtering, request-limit, and credential non-logging tests in `apps/coire-node/tests/unit/test_run_relay.py`
- [X] T026 [P] [US3] Write Docker integration confinement and memory/timeout tests in `tests/integration/test_container_runs.py`
- [X] T027 [US3] Implement a minimal gateway-only relay with `/v1` destination and method allowlists in `apps/coire-node/src/coire_node/run_relay.py`
- [X] T028 [US3] Create one internal network per run, attach only run and relay, publish no ports, and remove both attachments/network idempotently in `apps/coire-node/src/coire_node/runs.py`
- [X] T029 [US3] Enforce non-root, read-only rootfs, all capabilities dropped, no-new-privileges, PID/memory/CPU limits, tmpfs scratch, fixed mounts, and no restart in `apps/coire-node/src/coire_node/runs.py`
- [X] T030 [US3] Enforce wall-clock, log-byte, and result-byte ceilings with distinct terminal errors in `apps/coire-node/src/coire_node/runs.py` and `apps/coire-api/src/coire_scheduler/runs.py`
...
- [X] T032 [P] [US4] Write run-token mint/hash/scope/expiry/revocation/spend tests in `apps/coire-api/tests/unit/test_run_tokens.py`
- [X] T033 [P] [US4] Write gateway run-token authorization contract tests for model/tool/spend scope and admin-route denial in `apps/coire-api/tests/contract/test_run_token_auth.py`
- [X] T034 [P] [US4] Write admin kill contract and audit tests including unreachable-node behavior in `apps/coire-api/tests/contract/test_admin_runs.py`
- [X] T035 [US4] Implement opaque run-token minting, Argon2id verification, server-time expiry, atomic spend charge, and idempotent revocation in `apps/coire-api/src/coire_api/run_tokens.py`
- [X] T036 [US4] Integrate run-token principals and scope checks into `/v1` authentication/resolution without granting other routes in `apps/coire-api/src/coire_api/auth.py` and `apps/coire-api/src/coire_api/gateway/resolution.py`
- [X] T037 [US4] Implement admin list/kill routes that revoke before node contact and audit actor/time/outcome in `apps/coire-api/src/coire_api/routes/admin_runs.py`
- [X] T038 [US4] Implement idempotent node kill/remove and a scheduler kill workflow with a five-second responsive-node deadline in `apps/coire-node/src/coire_node/runs.py` and `apps/coire-api/src/coire_scheduler/runs.py`
...
- [X] T045 [P] Add `coire.node.run.*`, `coire.scheduler.run.*`, and `coire.api.run.*` spans, structured fields, bounded metrics, and content/token redaction across run paths
- [X] T046 [P] Add run lifecycle/capacity/kill/recovery dashboard panels and actionable stuck-run/orphan/log-limit alerts in the repository's provisioned observability paths
- [X] T047 [P] Add CLI run submit/list/show/kill/events commands and tests in `apps/coire-api/src/coire_api/cli.py` and `apps/coire-api/tests/unit/test_cli_runs.py`
- [X] T048 [P] Write operations, diagnosis, kill, cleanup, rollback, and credential-response procedures in `docs/runbooks/container-runs.md`
